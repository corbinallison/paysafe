 ...
 * is the mirror image of PaySafe's buyer-side scan: instead of protecting the
 ...
 */
export function handleTrustEvaluate(body: unknown, cfg: PaySafeConfig, store: Store): ApiResult {
  ...
  if (!subject.wallet) {
    return evaluation(
      ...
      50,
      "NO_WALLET_SUBJECT",
      "Query carried only a self-asserted agent_id; PaySafe keys payer history by wallet address. Include payer.wallet for a substantive evaluation.",
      300,
    );
  }
  ...
  if (store.badlist.has(wallet)) {
    return evaluation(
      ...
      0,
      "BADLISTED",
      "Subject wallet is on PaySafe's operator-curated known-bad list.",
      86_400,
      evidence,
    );
  }
