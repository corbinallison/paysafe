"""
crewai-paysafe — PaySafe payment security for CrewAI agents.
...
Give a CrewAI crew "scan before you pay" with two additions:
    ...
    paysafe = PaySafeClient(agent_id="my-agent")        # free key auto-minted
    register_paysafe_provenance(paysafe)                # <- the important one
    agent = Agent(role="buyer", tools=paysafe_tools(paysafe), ...)

Why the provenance registration matters more than the tools: PaySafe's
...
agent read. `register_paysafe_provenance` installs a CrewAI after-tool-call
...
learn what "provenance" means. PaySafe's own tool outputs are skipped so
...
Requires: crewai >= 0.100, paysafe-x402 >= 0.3.0. MIT.
PaySafe is advisory and non-custodial — never touches keys, wallets, or funds.
"""
...
try:
    ...
    raise ImportError(
        "crewai-paysafe requires crewai (pip install crewai). "
        "For plain-Python integrations use the paysafe-x402 package directly."
    ) from e

from paysafe_x402 import PaySafeBlockedError, PaySafeClient

__all__ = [
    "paysafe_tools",
    "register_paysafe_provenance",
    "guarded_payment",
    "PAYSAFE_TOOL_NAMES",
]
...
PAYSAFE_TOOL_NAMES = frozenset(
    ["paysafe_scan_payment", "paysafe_check_reputation", "paysafe_report_counterparty"]
)
...
class _ReputationTool(BaseTool):
    name: str = "paysafe_check_reputation"
    description: str = (
...
class _ReportTool(BaseTool):
    name: str = "paysafe_report_counterparty"
    description: str = (
...
def paysafe_tools(client: PaySafeClient) -> List[Any]:
    """The PaySafe CrewAI toolset (scan / check reputation / report), imperatively
...
# ---------------------------------------------------------------------------
def register_paysafe_provenance(client: PaySafeClient, max_chars: int = 8192) -> Callable[[Any], None]:
    """Install a global CrewAI after-tool-call hook that observes every tool
    output (except PaySafe's own) so the next scan is provenance-tagged — this
...
def guarded_payment(
    ...
    block verdict (or flag with strict=True) it raises PaySafeBlockedError and
    ...
    _guarded.__name__ = getattr(pay_fn, "__name__", "payment") + "_paysafe_guarded"
    return _guarded
