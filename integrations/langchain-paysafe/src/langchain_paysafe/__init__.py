"""
langchain-paysafe — PaySafe payment security for LangChain / LangGraph agents.
...
Two lines make an agent's x402 payments scanned by default:
    ...
    callbacks = [PaySafeProvenanceCallback(paysafe)]       # <- the important one
    # pass tools= and callbacks= to your agent as usual

Why the callback matters more than the tools: PaySafe's strongest detector
...
the model to consult a scan tool first), wrap its function:
    ...
    # safe_pay raises PaySafeBlockedError on a block verdict — the payment
    # function is never called. Build your payment tool from safe_pay.

Requires: langchain-core >= 0.3, paysafe-x402 >= 0.3.0. MIT.
...
try:
    ...
    raise ImportError(
        "langchain-paysafe requires langchain-core (pip install langchain-core). "
...
class PaySafeProvenanceCallback(BaseCallbackHandler):
    ...
    PaySafe's own tool outputs are skipped — a verdict echoed back as
    "content the agent read" would poison the provenance signal.
    """

    def __init__(self, client: PaySafeClient, max_chars: int = 8192):
        self._client = client
        self._max_chars = max_chars

    def _is_own_output(self, name: Optional[str], text: str) -> bool:
        if name and name in PAYSAFE_TOOL_NAMES:
            return True
...
def guarded_payment(
    ...
    PaySafeBlockedError and pay_fn is NEVER invoked — enforcement by
...
def paysafe_tools(client: PaySafeClient) -> List[Any]:
    """The PaySafe toolset, imperatively described so agents call them at the
    right moments (mirrors the PaySafe MCP server's tool framing)."""
