"""
The one-line diff: PaySafe in the default x402 payment path (Python parity
...
it settles:
    ...
    # use `guarded` anywhere a transport goes — e.g. PaySafeClient itself,
...
A block verdict raises PaySafeBlockedError BEFORE any payment is signed — the
...
__all__ = ["wrap_transport_with_paysafe", "payment_from_offer"]
...
def wrap_transport_with_paysafe(
    ...
    paysafe:           a PaySafeClient; its observe()/note_planning() state
    ...
    def guarded_transport(
        ...
        outgoing = paysafe.scan_outgoing(offer, expected_price_usd=expected)
        if on_scan:
            on_scan("outgoing", outgoing)
        if outgoing["verdict"] == "block" or (strict and outgoing["verdict"] == "flag"):
            raise PaySafeBlockedError(outgoing)

        # 2) The offer itself, as an incoming payment request.
        if scan_offer:
            incoming = paysafe.scan_incoming(offer, expected_price_usd=expected)
            if on_scan:
